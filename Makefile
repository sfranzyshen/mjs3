...
MFLAGS = -g -DMJS_DEBUG -O0 #-Os
TFLAGS = $(MFLAGS) -DMJS_PROP_POOL_SIZE=30 -DMJS_STRING_POOL_SIZE=200
all: $(PROG)
.PHONY: test $(PROG)
...
test: unit_test.c mjs.c mjs.h
	$(CC) -o $@ mjs.c unit_test.c $(CFLAGS) $(TFLAGS) && ./$@

test98: unit_test.c mjs.c mjs.h
	$(VC98) wine cl mjs.c unit_test.c /nologo /W4 /Os $(TFLAGS) /Fe$@.exe
	$(VC98) wine $@.exe
